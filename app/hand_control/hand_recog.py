...
def getStructuredLandmarks(landmarks):
    ...
    for j in range(42):
        if(j % 2 == 1):
            structuredLandmarks.append(
                {'x': landmarks[j - 1], 'y': landmarks[j]})
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        text2, memo = isSliding(results.multi_hand_landmarks[0], memo)[
            0], isSliding(results.multi_hand_landmarks[0], memo)[1]
        print("memo : "+str(memo[0])+" ,"+str(memo[1]))

        cv2.putText(image, text, (360, 360), font,
